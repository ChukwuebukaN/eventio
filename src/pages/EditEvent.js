...
function EditEvent() {
  ...
  useEffect(() => {
    ...
    document.title = "Eventio • Edit Event";
    try {
      dashboard.GetSpecificEvent().then((response) => {
  ...
  const fetchedEventsFunc = () => {
    <div className="event-id-text" key={events._id}>
      ...
      dashboard
        ...
        .then((response) => {
          ...
          if (response.status === 201) {
            console.log("👍 Event was deleted successfully", response);
            history.push(AuthRoutes.dashboard);
            console.log("👍 ROUTED");
          }
        })
  ...
  const showDeletBtn = () => {
    if (screenIsMobile === "true") {
      return (
        <MdDelete onClick={deleteEvent} className="deleteEvent-btn-icon" />
      );
    } else {
      return (
        <div onClick={deleteEvent}>
          <MdDelete className="deleteEvent-btn-icon-deskt" />
          <div className="deleteEvent-btn-text-deskt">DELETE EVENT</div>
        </div>
      );
    }
  };
  ...
  return (
    <div className="onboarding">
      ...
      {/* <div className="event-id-text"> DETAIL EVENT: #{fetchedEvents.id} </div> */}
      {fetchedEventsFunc()}
      {showDeletBtn()}
      <div
        className={
          screenIsMobile === "true" ? "editEvent-wrapper" : "editEvent-wrapper"
        }
      >
      ...
      {modalAccountModal()}
      <div className="editEvent-btn" onClick={handleEditEvent}>
        <FiCheck className="editEvent-btn-icon" />
      </div>
    </div>
  );
}
