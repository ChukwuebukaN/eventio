...
const EventsGridCard = ({ events }) => {
  ...
  /** Display Event Button based on user role on List Display */
  const displayEventUserButton = (fetchedEvents) => {
    ...
    if (fetchedEvents.owner.id === userId.id) {
      return (
        <div
          className={mobile ? "event-edit-button" : "event-edit-button-deskt"}
    ...
    if (fetchedEvents.owner.id !== userId.id) {
      return (
        <div
          className={mobile ? "event-join-button" : "event-join-button-deskt"}
          onClick
        >
          JOIN
        </div>
    ...
    if (fetchedEvents.attendees.id === userId.id) {
      return (
        <div
          className={mobile ? "event-leave-button" : "event-leave-button-deskt"}
          onClick
        >
          LEAVE
        </div>
  ...
  const displayFetchedEvents = () => {
    return events.map((fetchedEvents) => (
      <div
        className={mobile ? "events-list-item" : "events-list-item-deskt"}
        key={fetchedEvents._id}
      >
        <div className={mobile ? "event-list-title" : "event-list-title-deskt"}>
          {" "}
          {shortenTitleStrings(fetchedEvents.title)}{" "}
        </div>
        <div
          className={
            mobile ? "event-list-description-mobile" : "event-description-deskt"
          }
        >
          {" "}
          {shortenLongStrings(fetchedEvents.description)}{" "}
        </div>
        <div className={mobile ? "hidden" : "event-list-owner-deskt"}>
          {" "}
          {fetchedEvents.owner.firstName} {fetchedEvents.owner.lastName}{" "}
        </div>
        <div className={mobile ? "event-list-date" : "event-list-date-deskt"}>
          {" "}
          {getDateString(fetchedEvents.startsAt)}{" "}
        </div>
        <div
          className={
            mobile ? "event-list-capacity" : "event-list-capacity-deskt"
          }
        >
          {" "}
          <FaUser className="hidden" /> {fetchedEvents.attendees.length} of{" "}
          {fetchedEvents.capacity}{" "}
        </div>
  ...
  return (
    <div>
      <div className={mobile ? "events-list-display" : ""}>
        <div
          className={
            mobile ? "events-list-item-wrapper" : "events-list-item-wrapper"
          }
        >
